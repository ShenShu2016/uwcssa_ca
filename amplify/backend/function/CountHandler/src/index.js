 ...
 * @LastEditTime: 2022-06-18 23:03:07
...
exports.handler = async function (event) {
  ...
  for (let i = 0; i < event['Records'].length; i++) {
    ...
    if (
      ...
      (record.dynamodb.NewImage?.__typename.S === 'Like' &&
        record.dynamodb.NewImage?.eventLikesId?.S) ||
      (record.dynamodb.OldImage?.__typename.S === 'Like' &&
        record.dynamodb.OldImage?.eventLikesId?.S)
    ) {
      console.log('这里是like,并且是 Event的');
      let params;
      if (record.eventName === 'INSERT') {
        params = {
          Key: { id: { S: record.dynamodb.NewImage.likeCountId.S } },
      ...
      let targetTableParams;

      if (
        record.eventName === 'INSERT' &&
        record.dynamodb.NewImage.articleCommentsId?.S
      ) {
        targetTableParams = {
        ...
        record.eventName === 'INSERT' &&
        record.dynamodb.NewImage.eventCommentsId?.S
      ) {
        targetTableParams = {
          Key: { id: { S: record.dynamodb.NewImage.eventCommentsId.S } },
        ...
        targetTableParams = {
          Key: { id: { S: record.dynamodb.OldImage.eventCommentsId.S } },
          TableName: Event_Table,
        };
      }
      const result = await dynamodb.getItem(targetTableParams).promise();
      console.log(result);

      let params;
      if (record.dynamodb.NewImage.articleCommentsId?.S) {
        params = {
          Key: { id: { S: result.Item.articleCountId.S } },
          TableName: Count_Table,
        };
      } else if (record.dynamodb.NewImage.eventCommentsId?.S) {
        params = {
          Key: { id: { S: result.Item.eventCountId.S } },
          TableName: Count_Table,
        };
      }

...
export default function ArticleSubComments({ comment }) {
  ...
  return (
    <div>
      {Object.keys(comment.articleSubComments.items).length !== 0 ? (
          ...
          >
            {expanded ? (
              <Button variant="text" endIcon={<ArrowDropUpRoundedIcon />}>
                隱藏 {comment.articleSubComments.items.length} 回覆
              </Button>
            ) : (
              <Button variant="text" endIcon={<ArrowDropDownRoundedIcon />}>
                查看 {comment.articleSubComments.items.length} 回覆
              </Button>
            )}
          </AccordionSummary>
          <AccordionDetails>
            {comment.articleSubComments.items.map((ArticleSubComment) => {
              const { id, content, createdAt, userID, user } =
                ArticleSubComment;
              return (
                <Grid
                  container

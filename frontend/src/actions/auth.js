...
export const checkAuthenticated = () => async dispatch => {
    if (localStorage.getItem('access')) {
        ...
        try {
            const res = await axios.post(`${process.env.REACT_APP_API_URL}/auth/jwttoken/verify/`, body, config)
...
export const login = (email, password) => async dispatch => {
    ...
    try {
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/auth/jwttoken/`, body, config);
...
export const signup = (username, email, password1, password2) => async dispatch => {
    ...
    try {
        const res = await axios.post(`${process.env.REACT_APP_API_URL}/auth/registration/`, body, config);
...
export const verify = (key) => async dispatch => {
    ...
    try {
        await axios.get(`${process.env.REACT_APP_API_URL}/accounts/confirm-email/${key}/`, config);
